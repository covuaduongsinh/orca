...
export async function startSttDictation(
  ...
  try {
    await startSttSession(state, modelId, sink, hotwordsFilePath, owner, language)
...
async function startSttSession(
  ...
  if (manifest.provider === 'openai' || manifest.provider === 'groq') {
    ...
    const readApiKey = manifest.provider === 'groq' ? readGroqSpeechApiKey : readOpenAiSpeechApiKey
